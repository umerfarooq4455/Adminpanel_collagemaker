...
const CategoriesAdd: React.FC = () => {
  ...
  const CategoryDeti = async (id: number) => {
    ...
    try {
      ...
      console.log(response.data, 'dasdfas');
    } catch (err) {
      setError('Failed to fetch categories');
    } finally {
      setLoading(false);
    ...
    <>
      ...
      <div className="rounded-[10px] border border-stroke bg-white px-2 pt-2 pb-2.5  shadow-default dark:border-strokedark dark:bg-boxdark  xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          ...
          {loading ? (
            <div className="flex mx-2 py-3 items-center  bg-white">
                ...
                {categories.map((catItem: any) => (
                  <tr>
                    ...
                    <td className="text-end py-5 px-4 dark:border-strokedark">
                      <p
                        className={`font-semibold flex items-center text-end text-[16px] ${catItem.is_active == 1 ? 'text-[#48eb48]' : 'text-[#FF0000] dark:text-[#FF0000]'} dark:text-[#48eb48]`}
                      >
                    ...
                    <td className="  py-5 px-4 flex justify-end dark:border-strokedark">
                      <div className="flex items-center ">
                        ...
                        <button
                          ...
                          onClick={() => {
                            CategoryDeti(catItem.cat_id), openModalEdit();
                          }}
                        >
                          <svg
                            className="fill-current"
